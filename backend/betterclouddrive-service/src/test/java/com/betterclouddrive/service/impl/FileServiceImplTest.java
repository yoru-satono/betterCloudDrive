...
class FileServiceImplTest {
    ...
    void listFiles_shouldReturnPage() {
        ...
        Page<FileEntity> page = new Page<>(1, 20);
        page.setRecords(List.of(file1, file2));
        page.setTotal(2);
        when(fileMapper.selectPage(any(Page.class), any(LambdaQueryWrapper.class))).thenReturn(page);
    ...
    void createFolder_shouldSucceed() {
        when(fileMapper.selectCount(any(LambdaQueryWrapper.class))).thenReturn(0L);
    ...
    void createFolder_shouldThrowWhenNameConflict() {
        when(fileMapper.selectCount(any(LambdaQueryWrapper.class))).thenReturn(1L);
    ...
    void copyFile_shouldAppendCopySuffix() {
        FileEntity source = FileEntity.builder()
                ...
                .isDeleted(false)
                .build();
        when(fileMapper.selectById(1L)).thenReturn(source);
